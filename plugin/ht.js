let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    conn.sendFile(m.chat, img, 'Harta Tahta.png', '*Nih bro jangan lupa follow Ig kami @ff.kelvin15*', m)
...
function ht(text = '') {
  ...
  return new Promise((resolve, reject) => {
    ...
    let o = 1 * Tanggal baru + '_harta_tahta.png'
    ...
    .on ('close', () => {
      ...
      coba {
        ...
        menyelesaikan (fs.readFileSync (o))
...
function noise (_var, depth = 4, s = 1024, freq) {
  ...
  untuk (let i = 0; i <depth; i ++) forms.push (

    rumus(

      _var,

      frekuensi * rand (40, 80) * (s / 2048) / s * ((i + 1) / 5),
...
rumus fungsi (_var, freq, offset, amp, add) {

  return `($ {add.toFixed (3)} + $ {amp.toFixed (4)} * sin ($ {offset.toFixed (6)} + 2 * PI * $ {_ var} * $ {freq.toFixed ( 6)})) `

}

function textArgs (teks, latar belakang, warna, ukuran, fontfile, x = '200', y = '200', w = 1024, h = 1024) {

  kembalikan `color = $ {background}: s = $ {w} x $ {h}, drawtext = text = '$ {text.replace (/ [\\] / g,' \\ $ & ')}': fontfile = '$ {fontfile.replace (/ [\\] / g,' \\ $ & ')}': x = $ {x}: y = $ {y}: fontsize = $ {size}: fontcolor = $ {color} `

}

function pickRandom (daftar) {

  daftar kembali [Math.floor (Math.random () * list.length)]
